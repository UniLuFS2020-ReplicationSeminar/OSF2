...
guardian_amazon <- df
guardian_amazon$wordcount <- as.numeric(guardian_amazon$wordcount)

# Remove rows that are not articles
guardian_amazon <- guardian_amazon %>% 
  filter(wordcount > 20)


# Create Corpus --------------------------------------------------------------

# Only include articles that reference Amazon as a company using Named Entity Recognition by spacy
library("spacyr")
...
for (row in 1:nrow(guardian_amazon_sample)) {
  parsedtxt_temp <- spacy_parse(guardian_amazon_sample$body_text[row]) # Tag text
  ...
  if (nrow(parsedtxt_temp) > 0) { # Check if there are any rows left after filtering
    parsedtxt_temp$doc_id <- guardian_amazon_sample$X[row] # Keep track of text IDs
...
guardian_amazon_sample <- guardian_amazon_sample %>% 
